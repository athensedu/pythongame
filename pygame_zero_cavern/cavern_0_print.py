...
# ======== add these declarations before running ==============
screen: 'pgzrun.Screen'
keyboard: 'pgzrun.keybord'
music: 'pgzrun.music'
sounds: 'pgzrun.sounds'
...
if pgzero_version < [1, 2]:
    print(
        ...
        "Please upgrade using the command 'pip3 install --upgrade pgzero'".format(
            pgzero.__version__))
    sys.exit()
    ...
    def update(self):
        ...
        if self.timer == self.blown_frames:
            ...
            if self.trapped_enemy_type != None:
                # trapped_enemy_type is either zero or one. A value of one means there's
                # a chance of creating a
        ...
        if self.timer < 9:
            # Orb grows to full size over the course of 9 frames -
            # the animation frame updating every 3 frames
...
class Bolt(CollideActor):
    ...
    def update(self):
        ...
        if self.move(self.direction_x, 0, Bolt.SPEED):
            ...
            # We didn't collide with a block -
            # check to see if we collided with an orb or the player
...
class GravityActor(CollideActor):
    ...
    def update(self, detect=True):
        ...
        # The detect parameter indicates whether we should check for collisions
        # with blocks as we fall. Normally we
        # want this to be the case - hence why this parameter is optional,
        # and is True by default. If the player is
        # in the process of losing a life, however, we want them to just fall
        # out of the level, so False is passed in this case.
        if detect:
            ...
            # just update the Y coordinate without any further checks
            self.y += self.vel_y
...
class Fruit(GravityActor):
    ...
    def __init__(self, pos, trapped_enemy_type=0):
        ...
        if trapped_enemy_type == Robot.TYPE_NORMAL:
            ...
            # If trapped_enemy_type is 1, it means this fruit came from
            # bursting an orb containing the more dangerous
            # type of enemy. In this case there is a chance of getting
            # an extra help or extra life power up
            # We create a list containing the possible types of fruit,
            # in proportions based on the probability we want
...
class Player(GravityActor):
    def __init__(self):
        ...
        # Initial pos is 0,0 but reset is always called straight afterwards which
    ...
    def hit_test(self, other):
        # Check for collision between player and bolt -
        # called from Bolt.update. Also check hurt_timer - after being hurt,
        # there is a period during which the player cannot be hurt again
        if self.collidepoint(other.pos) and self.hurt_timer < 0:
            # Player loses 1 health, is knocked in the direction
            # the bolt had been moving, and can't be hurt again
            # for a while
    ...
    def update(self):
        # Call GravityActor.update - parameter is whether
        # we want to perform collision detection as we fall. If health
        ...
        if self.hurt_timer > 100:
            # We've just been hurt. Either carry out the sideways motion
            # from being knocked by a bolt, or if health is
            # zero, we're dropping out of the level, so check for
            # our sprite reaching a certain Y coordinate before
            # reducing our lives count and responding the player.
            # We check for the Y coordinate being the screen height
            # plus 50%, rather than simply the screen height,
            # because the former effectively gives us a short delay
            ...
            if space_pressed() and self.fire_timer <= 0 and len(game.orbs) < 5:
                ...
                # by Athensoft - change x distance
                x = min(730, max(70, self.x + self.direction_x * 38))
                y = self.y - 35
...
class Robot(GravityActor):
    ...
    def update(self):
        ...
        # The more powerful type of robot can deliberately shoot at orbs -
        # turning to face them if necessary
        ...
        if self.fire_timer >= 12:
            # Random chance of firing each frame. Likelihood increases 10 times
            # if player is at the same height as us
            ...
            #  Once the fire timer has been set to 0, it will count up -
            #  frame 8 of the animation is when the actual bolt is fired
...
class Game:
    ...
    def fire_probability(self):
        # Likelihood per frame of each robot firing a bolt -
        # they fire more often on higher levels
        return 0.001 + (0.0001 * min(100, self.level))

    def max_enemies(self):
        # Maximum number of enemies on-screen at once -
        # increases as you progress through the levels
    ...
    def next_level(self):
        ...
        # Note that we don't do 'self.grid.append(self.grid[0])'.
        # That would alter the original data in the LEVELS list
        # Instead, what this line does is create a brand new list,
        # which is distinct from the list in LEVELS, and
        # consists of the level data plus the first row of the level.
        # It's also interesting to note that you can't
        # do 'self.grid += [self.grid[0]]', because that's equivalent to using append.
        # As an alternative, we could have copied the list on the line
        # below '# Set up grid', by writing
        ...
        # At the start of each level we create a list of pending enemies -
        # enemies to be created as the level plays out.
        # When this list is empty, we have no more enemies left to create,
        # and the level will end once we have destroyed
        # all enemies currently on-screen. Each element of the list will be either 0 or 1,
        ...
        # Then we create the list of pending enemies, using Python's ability to
        # create a list by multiplying a list
        # by a number, and by adding two lists together. The resulting list
        # will consist of a series of copies of
        # the number 1 (the number depending on the value of num_strong_enemies),
        # followed by a series of copies of
        ...
        # Finally we shuffle the list so that the order is
        # randomised (using Python's random.shuffle function)
    ...
    def get_robot_spawn_x(self):
        ...
        for i in range(NUM_COLUMNS):
            # Keep looking at successive columns (wrapping round
            # if we go off the right-hand side) until
        ...
        # If we failed to find an opening in the top grid row (shouldn't ever happen),
        # just spawn the enemy
    ...
    def update(self):
        ...
        # Use list comprehensions to remove objects which are no longer wanted
        # from the lists. For example, we recreate
        # self.fruits such that it contains all existing fruits except
        # those whose time_to_live counter has reached zero
        ...
        # Every 100 frames, create a random fruit (unless there are
        # no remaining enemies on this level)
        ...
        # Every 81 frames, if there is at least 1 pending enemy,
        # and the number of active enemies is below the current
        ...
        # End level if there are no enemies remaining to be created,
        # no existing enemies, no fruit, no popping orbs,
        # and no orbs containing trapped enemies.
        # (We don't want to include orbs which don't contain trapped enemies,
    ...
    def play_sound(self, name, count=1):
        # Some sounds have multiple varieties. If count > 1,
        # we'll randomly choose one from those
        ...
        if self.player:
            try:
                ...
                # from the sounds folder (if
                # such a file exists)
                # But what if you have files named 'explosion0.ogg' to 'explosion5.ogg'
                # and want to randomly choose
                # one of them to play? You can generate a string such as 'explosion3',
                # but to use such a string
                # to access an attribute of Pygame Zero's sounds object,
...
def char_width(char):
    # Return width of given character. For characters other than
    # the letters A to Z (i.e. space, and the digits 0 to 9),
    # the width of the letter A is returned. ord gives the
    # ASCII/Unicode code for the given character.
...
def draw_text(text, y, x=None):
    if x == None:
        # If no X pos specified, draw text in centre of the screen -
        # must first work out total width of text
...
def draw_status():
    ...
    # We only display a maximum of two lives - if there are more than two,
...
def update():
    ...
    if state == State.MENU:
        if space_pressed():
            # Switch to play state, and create a new Game object,
            # passing it a new Player object to use
            state = State.PLAY
            game = Game(Player())
        else:
            game.update()
...
def draw():
    ...
    if state == State.MENU:
        ...
        # We enclose this calculation in the min function,
        # with the other argument being 9, which results in the
        # animation staying on frame 9 for three quarters of the time.
        # Adding 40 to the game timer is done to alter
